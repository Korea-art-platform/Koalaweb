...
export default function AccountOrders() {
  ...
  useEffect(() => {
    ...
    getMyOrders(page, 10)
      ...
      .catch((e) => console.error('주문 내역 로딩 실패:', e))
  ...
  const handleCancel = async (orderNo: string) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.orderNo === orderNo ? { ...o, orderStatus: 'CANCELLED' } : o))
      );
    } catch (e: any) {
    ...
    <>
      <div className="mb-6 md:mb-8 px-1">
        ...
        <p className="text-xs md:text-sm text-gray-400 font-medium">
          {t('order.history.totalCount', { count: orders.length })}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4 md:space-y-6">
          {orders.map((order) => {
            const colorClass = STATUS_COLORS[order.orderStatus] ?? 'bg-gray-50 text-gray-600 border-gray-100';
            const statusLabel = t(`order.status.${order.orderStatus}`, { defaultValue: order.orderStatus });
            return (
              <div
                key={order.orderNo}
                ...
                <div className="px-5 py-4 md:px-8 md:py-5 bg-gray-50/50 border-b border-gray-100 flex flex-wrap justify-between items-center gap-y-3">
                  <div className="flex gap-4 md:gap-8 overflow-x-auto no-scrollbar">
                    <div className="shrink-0">
                      <p className="text-[9px] md:text-[10px] text-gray-400 uppercase mb-0.5 font-bold">{t('order.detail.orderNo')}</p>
                      <p className="text-[12px] md:text-sm font-black text-gray-900">{order.orderNo}</p>
                    </div>
                    <div className="shrink-0">
                      <p className="text-[9px] md:text-[10px] text-gray-400 uppercase mb-0.5 font-bold">{t('order.detail.orderDate')}</p>
                      <p className="text-[12px] md:text-sm font-bold text-gray-700">
                        {new Date(order.createdAt).toLocaleDateString('ko-KR')}
                      </p>
                    </div>
                    <div className="shrink-0">
                      <p className="text-[9px] md:text-[10px] text-gray-400 uppercase mb-0.5 font-bold">{t('order.detail.totalAmount')}</p>
                      <p className="text-[12px] md:text-sm font-black text-black">
                        ₩{order.totalAmount.toLocaleString()}
                      </p>
                    </div>
                  </div>
                  <span className={`px-2.5 py-1 rounded-full text-[10px] md:text-[11px] font-black border uppercase tracking-tighter ${colorClass}`}>
                    {statusLabel}
                  </span>
                </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {order.itemCount > 1 && (
                        <span className="text-[9px] md:text-[10px] text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded font-bold">
                          {t('order.detail.otherItems', { count: order.itemCount - 1 })}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 mt-8 pt-6 border-t border-gray-50">
                    {order.orderStatus === 'SHIPPED' && (
                      <button className="flex items-center justify-center gap-2 px-5 py-3 bg-black text-white rounded-xl hover:bg-gray-800 transition-all text-xs font-bold">
                        <Box className="w-4 h-4" /> {t('order.history.trackShipping')}
                      </button>
                    )}
