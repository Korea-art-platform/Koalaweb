    ...
    <section className="py-8 md:py-20 px-4 md:px-12">
      <div className="max-w-[1800px] mx-auto">
        <div className="flex items-end justify-between mb-5 md:mb-12">
          <div>
            <h2 className="text-2xl md:text-4xl font-bold tracking-tight mb-1 md:mb-2">{t('home.categories.title')}</h2>
            <p className="text-gray-400 text-xs md:text-base font-medium">{t('home.categories.subtitle')}</p>
          </div>
          ...
          {categories.map((category) => (
            <Link key={category.id} to={`/smart-store?category=${category.id}`} className="flex-shrink-0 group">
              <div className="px-5 md:px-8 py-4 md:py-6 bg-gray-50 rounded-2xl border border-transparent group-hover:border-black group-hover:bg-white transition-all duration-300 min-w-[140px] md:min-w-[200px]">
                ...
                <p className="text-[10px] md:text-xs text-gray-400 font-medium uppercase tracking-widest">{category.count} {t('home.categories.items')}</p>
              </div>
            </Link>
          ))}
        </div>
